<script>
    ...
    import {
        getSortedElectionsFromRiding,
        getSortedElectionRidingRuns,
    } from '$lib/stats.js';
    ...
    $: electionData = getSortedElectionsFromRiding($page.params.ridingId).toReversed().map(
        (electionId) => {
            const election = elections[electionId];
            return {
                ...election,
                barData: getSortedElectionRidingRuns(electionId, $page.params.ridingId).map(
                    (run) => {
...
<Page>
    ...
    <SubTitle text={PROVINCES[riding.province] + ", " + riding.start_date.year + "–" + (riding.end_date ? riding.end_date.year : "")} />

    <div class="flex flex-col-reverse sm:flex-row justify-center w-full mt-6 sm:mt-8 space-x-0 space-y-4 space-y-reverse sm:space-y-0 sm:space-x-8">
        {#if riding.summary}
            <div class="grow">
                <Paragraph wikiLink={riding.summary_url} paragraphs={riding.summary} />
            </div>
        {:else}
