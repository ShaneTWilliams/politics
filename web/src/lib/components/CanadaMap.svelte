<script>
    ...
    import { formatString, isSm, arrayRange } from "$lib/utils.js"
    ...
    $: allDoneLoading = mainMapDoneLoading &&
...
<div class="w-full">
    ...
    <div class={`w-full ${allDoneLoading ? "" : "opacity-0 h-0"}`}>
        ...
        <div class="pb-4 mt-8 sticky -bottom-[1px]" bind:this={stickyElement}>
            <div class={`flex flex-col items-center bg-sol-light2 dark:bg-sol-dark2 p-2 pb-4 rounded-lg w-full ${stuck ? "shadow-[rgba(0,0,0,0.15)_0px_0px_30px_10px]" : ""}`}>
                <div class="flex flex-row w-full mb-2 px-4 pt-2 text-sm sm:text-md">
                    <div class="flex flex-col items-center w-full grow">
                        <a class="font-black hover:underline decoration-dashed" href={`/elections/ridings/${selectedRiding}`}>
