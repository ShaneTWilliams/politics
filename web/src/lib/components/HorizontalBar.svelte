<script>
    ...
    export let data, showTotal, minimumPercent = 0, pct=false;
...
<div class="flex flex-col items-start sm:items-end w-full text-xs overflow-y-scroll text-nowrap">
    {#if data}
    <table class="table-auto border-separate border-spacing-x-3 w-full min-w-0">
        {#each data as row}
        <tr>
            <td class="space-y-1 flex-col flex text-left truncate align-middle max-w-max sm:max-w-48 lg:max-w-max">
                {#if row.primaryLink}
                <a href={row.primaryLink} class="hover:underline decoration-dashed">
                    <p class="truncate">
                        {row.primaryLabel}
                    </p>
                </a>
            ...
            <td class="hidden sm:table-cell">
                ...
                <a href={row.secondaryLink} class="text-left font-bold h-5 hover:underline decoration-dashed">
                    {row.secondaryLabel}
                </a>
                {:else}
                <p class="font-bold h-5 hidden sm:block">
                    {row.secondaryLabel}
                </p>
                {/if}
            </td>
            {/if}
            {#if min_count < 0}
            <td class="rounded h-5 border border-sol-light2 dark:border-sol-dark2 border-2 hidden sm:table-cell w-full">
                <div class="flex flex-row w-full h-full">
                    <div class="h-full flex flex-row justify-end" style={`width: ${min_count * 100 / (min_count - max_count)}%`}>
                        {#if row.count < 0}
                        <div
                    ...
                    <div class="h-full flex flex-row" style={`width: ${max_count * 100 / (max_count - min_count)}%`}>
                        {#if row.count > 0}
                        <div
                            class={`h-full rounded hover:opacity-80 ${FILL_COLOURS[row.color]}`}
