...
import { PARTIES_THAT_ARENT_PARTIES, OCCUPATIONS_THAT_ARENT_OCCUPATIONS, RO_YEARS } from '$lib/constants';
...
export function getPartyNumberOfMPsCurrently(partyId) {  // TODO: Fix this function.
...
export function getPreviousGeneralElection(electionId) {
    ...
    for (const [id, election] of Object.entries(elections)) {
        if (dateLaterThan(thisElectionDate, election.date)
...
export function getElectionGeographyForView(electionId, viewId) {
    ...
    for (const runId of election.runs) {
        ...
        if (view != undefined && (!(ro_year in view.ridings_by_year) || !(view.ridings_by_year[ro_year].includes(run.riding)))) {
            continue;
        }
